...
| **1.4**  | `web` zerschneiden **gemäß Spike Rev. 2**: Runtime → way2go (`web.go`, `screen.go`, `page.go`, `dev.go`, `module.go`-Gruppe-a); Builder-Rest → `package stack` (`builder.go` minus Routing, `assets.go`, `module.go`-Gruppe-b als Asset-Parts); `app.go`→stack reshaped (`runtime`+`builder`, `Handler()`+`toAssetLinks`); `RouteAccumulator`/`Registrar`/`AssetLinks` neu in way2go. | **Sechs Dateien + `stack.go`** berührt (`builder.go`, `web.go`, `page.go`, `dev.go`, `module.go`-Split, `internal/build/build.go`); kein Modul-Zyklus (way2go sieht kein `AssetRef`); `go build ./...` in beiden Modulen grün. |
